...
var VivogamePlugin = (function () {
    ...
    VivogamePlugin.prototype.onFinish = function (pluginContext) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            var gameJSPath = path.join(pluginContext.outputDir, "game.js");
            ...
            var engineVersion = this.readData(path.join(pluginContext.projectRoot, "egretProperties.json")).engineVersion;
            ...
            var isPublish = pluginContext.buildConfig.command == "publish" ? true : false;
            var configArr = [];
            for (var i = 0, len = this.jsFileList.length; i < len; i++) {
                var jsFile = this.jsFileList[i];
                if (isPublish) {
                    if (jsFile == "main.js") {
                        jsFile = 'main.min.js';
                    }
            ...
            var replaceConfigStr = '\/\/----auto option start----\n\t\t' + configArr.toString() + '\n\t\t\/\/----auto option end----';
            var minigameConfigPath = path.join(pluginContext.outputDir, "../", "minigame.config.js");
            if (!fs.existsSync(minigameConfigPath)) {
                ...
                var configJSContent = fs.readFileSync(minigameConfigPath, { encoding: "utf8" });
